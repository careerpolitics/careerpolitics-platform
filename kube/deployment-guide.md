...
# 7️⃣ **Configure DigitalOcean Spaces (S3-compatible Storage)**

## Create a Space

1. DO → **Spaces**
2. Create Space
3. Region: **sgp1** (or closest)
4. Name: `careerpolitics-images`

## Create Spaces Access Keys

DigitalOcean → API → **Spaces Keys** → Generate
...
Enabling CDN gives a faster delivery endpoint.

## Optional: Set CORS

```bash
doctl spaces cors put careerpolitics-images \
  --cors-rules '[ { "AllowedOrigins": ["https://careerpolitics.com"], "AllowedMethods": ["GET","PUT","POST","DELETE","HEAD"], "AllowedHeaders": ["*"], "ExposeHeaders": ["ETag"], "MaxAgeSeconds": 3000 } ]'
```
